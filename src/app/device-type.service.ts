...
import { DeviceType, DeviceTypeDetails, DtManifest } from './device-type';
...
//{"data":{"id":"fitbit_flex","oid":"8d62e034c76247809ee48ff456fbaa65","uid":"10024","name":"zz_OLD_fitbit_flex","published":true,"approved":true,"protected":true,"latestVersion":3,"uniqueName":"fitbit_flex","vid":"0","rsp":false,"issuerDn":null,"description":null,"inStore":false,"hasCloudConnector":true,"latestCloudConnectorVersion":1,"billingSubscriptionId":"sub_7iQ94o25ZxWocR","lastUpdated":1454420127000,"ownedByCurrentUser":false}}
  findDeviceType(id: string): Observable<DeviceTypeDetails> {
    return this.accessTokenService.getPublicAccessToken()
    .switchMap( token => {
      let headers = new Headers({'Content-Type': 'application/json', 'Authorization' : `bearer ${token}`});
      return this.http.get(`${environment.api.rest}devicetypes/${id}?productInfo=true`, {headers: headers}).map((v: Response) => {return {token: token, response: v}})
    })
    //.do((r) => console.log("resp", r.json()))
    .switchMap( v  => {
      let dt = v.response.json().data;
      let headers = new Headers({'Content-Type': 'application/json', 'Authorization' : `bearer ${v.token}`});
      return this.http.get(`${environment.api.rest}devicetypes/${id}/manifests/latest/properties`, {headers: headers})
        .map((r: Response) => {
          dt.manifest = r.json().data as DtManifest;
          return dt as DeviceTypeDetails;
        })
    })
    // .toPromise()
    ;
